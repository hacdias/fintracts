...
- [Composite Agreements](#composite-agreements)
  - [Currency Swap with Interest Rate Swap](#currency-swap-with-interest-rate-swap)
...
### Currency Swap with Interest Rate Swap

A currency swap can be written together with an interest rate swap, where each
borrower pays interest over the money they borrowed. Only two interest clauses are
allowed in this case and it must be a clause per party.
